...
import { Key, Bell, User, ShieldCheck, ScrollText } from "lucide-react";
import PermissionWrapper from "../auth/PermissionWrapper";

interface SettingsSidebarProps {
    currentTab: 'api' | 'general' | 'users' | 'security' | 'audit';
    setCurrentTab: (tab: 'api' | 'general' | 'users' | 'security' | 'audit') => void;
}
...
export function SettingsSidebar({ currentTab, setCurrentTab }: SettingsSidebarProps) {
    const tabs = [
        ...
        { id: 'audit', label: 'Sistem Günlükleri', icon: <ScrollText size={18} /> },
        ...
        <div className="lg:col-span-1 space-y-2">
            {tabs.map((tab) => (
                <React.Fragment key={tab.id}>
                    {(tab.id === 'users' || tab.id === 'general' || tab.id === 'audit') ? (
