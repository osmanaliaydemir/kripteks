    ...
    return (
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                ...
                <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <h2 className="text-3xl font-bold font-mono text-white mb-1">
                ...
                <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <h2 className={`text-3xl font-bold font-mono mb-1 ${wallet && wallet.total_pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
            ...
            <div className="flex gap-4 mb-8">
                <button onClick={() => toast.info("Para Yatırma modülü yakında eklenecek.")} className="px-6 py-3 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 rounded-xl font-bold flex items-center gap-2 transition-all">
                    <ArrowDownLeft size={18} />
                    Para Yatır
                </button>
                <button onClick={() => toast.info("Para Çekme modülü yakında eklenecek.")} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 border border-white/5 text-slate-300 rounded-xl font-bold flex items-center gap-2 transition-all">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white/[0.02] border-b border-white/5">
                                ...
                                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">İşlem Tipi</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {transactions.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="p-12 text-center text-slate-500 text-sm">Henüz işlem geçmişi yok.</td>
                                </tr>
                            ) : (
                                transactions.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-white/[0.02] transition-colors">
                                        <td className="p-4 text-sm text-slate-400 font-mono w-40">{new Date(tx.createdAt).toLocaleString()}</td>
                                        <td className="p-4 w-40">
                                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${tx.amount >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'
                                                }`}>
                                                {tx.type}
                                            </span>
                                        </td>
                                        <td className="p-4 text-sm text-slate-300">{tx.description}</td>
                                        <td className={`p-4 text-sm font-bold font-mono text-right w-32 ${tx.amount >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
