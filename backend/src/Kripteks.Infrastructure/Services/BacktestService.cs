...
public class BacktestService
{
    ...
    public async Task<BacktestResultDto> RunBacktestAsync(BacktestRequestDto request)
    {
        ...
        var startTime = DateTime.UtcNow.AddDays(-7); // Default 7 gün
        ...
        {
            // Binance'den çek
            var klines = await _client.SpotApi.ExchangeData.GetKlinesAsync(symbol, interval, startTime: currentStartTime, limit: 1000);
        ...
        _logger.LogInformation("Backtest Veri Tamamlandı: {Symbol} | Toplam {Count} mum çekildi.", symbol, allCandles.Count);
